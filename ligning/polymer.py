...
class PolymerGraph():
    ...
    def find_available_C1_in_polymer(self, branching_state: Optional[bool] = None) -> list:
        ...
        if branching_state is not None:
            C1_indices_in_polymer = self.update_branching_C_in_polymer(
                C1_indices_in_polymer, branching_state)

        return C1_indices_in_polymer
    
    def find_available_O_pairs_indicies_in_polymer(self):

        C_pair_indices_in_polymer = [ei for ei in self.G.edges if self.G.edges[ei]['btype'] == '5-5']
        C1_indices_in_polymer = self.find_available_C1_in_polymer()
        O_pair_indices_in_polymer = []
        # Check if the neighbor 4th Carbon of the 5th carbon is available
        for C_pair_i_in_polymer in C_pair_indices_in_polymer:
            C1_4th_in_polymer = C_pair_i_in_polymer[0]-1
            C2_4th_in_polymer = C_pair_i_in_polymer[1]-1
            if (C1_4th_in_polymer in C1_indices_in_polymer) and (C2_4th_in_polymer in C1_indices_in_polymer):
                # Adjust the index of O by adding 5 to the 5th Carbon index
                O_pair_indices_in_polymer.append((C_pair_i_in_polymer[0]+5, C_pair_i_in_polymer[1]+5))

        return O_pair_indices_in_polymer
    ...
    def update_available_C1_in_polymer(self, C1_index_in_polymer: int) -> list:
        ...
        C1_indices_in_polymer = [
            n for n in self.C1_indices_in_polymer if not n == C1_index_in_polymer]
    ...
    def find_available_C2_in_monomer(self, C1_index_in_polymer: int, ring: Optional[bool] = False) -> list:
        ...
        C2_indices_in_monmer = monomer_select_C1_C2[C1_node['mtype']
                                                    ][C1_node['index']]

        if ring:
            C2_indices_in_monmer = [
                ci for ci in C2_indices_in_monmer if ci != 1]
    ...
    def find_O_index_in_polymer(self, C_index_in_polymer: int) -> int:
        ...
        """
        O_index_in_polymer = [i for i in list(self.G.neighbors(
            C_index_in_polymer)) if (self.G.nodes[i]['element'] == 'O')][0]
    ...
    def connect_C1_C2_O_pair(
        ...
        self.G.add_edges_from([linkage_new_index_1], order=1, index=linkage_new_1,
                              mtype=linkage_new_types_1, btype=linkage_new_name)
        self.G.add_edges_from([linkage_new_index_2], order=1, index=linkage_new_2,
                              mtype=linkage_new_types_2, btype=linkage_new_name)
        
        # Update the original 5-5 bond to the new linkage name
        if (O1_index_in_polymer-5, O2_index_in_polymer-5) in self.G.edges:
            self.G.edges[(O1_index_in_polymer-5, O2_index_in_polymer-5)]['btype'] = linkage_new_name
        ...
        if self.verbose:
            print('Connect a {} to a {} unit (in polymer) via a {} bond'.format(
                linkage_new_types_1, linkage_new_types_1, linkage_new_name))
            print('Connect a {} to a {} unit (in polymer) via a {} bond'.format(
                linkage_new_types_2, linkage_new_types_2, linkage_new_name))
    ...
    def connect_C1_C2(
        ...
        if linkage_new_name in linkage_special_names:
            ...
            if 4 in linkage_new:
                ...
                if linkage_new[0] == 4:  # C1 is the 4th carbon
                    O_index_in_polymer = self.find_O_index_in_polymer(
                        C1_index_in_polymer)
                ...
                linkage_new_index_C1_O = (
                    C1_index_in_polymer, O_index_in_polymer)
                linkage_new_index_O_C2 = (
                    O_index_in_polymer, C2_index_in_polymer)
                linkage_new_index_list = [
                    linkage_new_index_C1_O, linkage_new_index_O_C2]
            ...
            if linkage_new_name == 'beta-O-4':
                ...
                # index to -OH that needs to be added at alpha(7th) of the first monomer
                O_index_in_polymer = len(self.G)
                ...
                linkage_new_index_alpha2_O = (
                    alpha2_index_in_polymer, O_index_in_polymer)
            ...
            if linkage_new_name == 'beta-5':
                ...
                if linkage_new[0] == 8:  # C1 is the beta 8th carbon, C2 is the 5th carbon
                    O_index_in_polymer = self.find_O_index_in_polymer(
                        C2_index_in_polymer-1)  # -1 to adjust to the 4th carbon
                ...
                if linkage_new[1] == 8:  # C1 is the 5th carbon, C2 is the beta 8th carbon
                    O_index_in_polymer = self.find_O_index_in_polymer(
                        C1_index_in_polymer-1)  # -1 to adjust to the 4th carbon
                ...
                if self.G.nodes[C4_index_in_polymer]['bonding'] and self.G.nodes[alpha_index_in_polymer]['bonding']:
                    linkage_new_index_C1_C2 = (
                        C1_index_in_polymer, C2_index_in_polymer)
                    linkage_new_index_O_alpha = (
                        O_index_in_polymer, alpha_index_in_polymer)
                    linkage_new_index_list = [
                        linkage_new_index_C1_C2, linkage_new_index_O_alpha]
            ...
            if linkage_new_name == 'beta-beta':
                ...
                O1_index_in_polymer = self.find_O_index_in_polymer(
                    C1_index_in_polymer+1)  # +1 to adjust to the 9th carbon
                O2_index_in_polymer = self.find_O_index_in_polymer(
                    C2_index_in_polymer+1)  # +1 to adjust to the 9th carbon

                alpha1_index_in_polymer = C1_index_in_polymer - \
                    1  # -1 to adjust to the alpha(7th) carbon
                alpha2_index_in_polymer = C2_index_in_polymer - \
                    1  # -1 to adjust to the alpha(7th) carbon
                ...
                if self.G.nodes[alpha1_index_in_polymer]['bonding'] and self.G.nodes[alpha2_index_in_polymer]['bonding']:
                    linkage_new_index_C1_C2 = (
                        C1_index_in_polymer, C2_index_in_polymer)
                    linkage_new_index_alpha1_O2 = (
                        alpha1_index_in_polymer, O2_index_in_polymer)
                    linkage_new_index_alpha2_O1 = (
                        alpha2_index_in_polymer, O1_index_in_polymer)

                    linkage_new_index_list = [
                        linkage_new_index_C1_C2, linkage_new_index_alpha1_O2, linkage_new_index_alpha2_O1]
            ...
            if linkage_new_name == 'beta-1':
                ...
                if linkage_new[0] == 8:  # C1 is the beta 8th carbon, C2 is the 1th carbon
                    ...
                    # the atoms connect to this needs to be deleted
                    alpha2_index_in_polymer = C2_index_in_polymer-1
                ...
                atom_indices_to_delete_in_polymer = [
                    ni + alpha2_index_in_polymer for ni in atom_indices_to_delete]
                ...
                linkage_old_index_alpha_beta = (
                    alpha1_index_in_polymer, alpha2_index_in_polymer)
                # set the alpha-beta bond order back to 1
                self.G.edges[linkage_old_index_alpha_beta]['order'] = 1

                linkage_new_index_C1_C2 = (
                    C1_index_in_polymer, C2_index_in_polymer)
                linkage_new_index_alpha1_O = (
                    alpha1_index_in_polymer, O_index_in_polymer)
                linkage_new_index_list = [
                    linkage_new_index_C1_C2, linkage_new_index_alpha1_O]
        ...
        if len(linkage_new_index_list) > 0:
            self.G.add_edges_from(linkage_new_index_list, order=1, index=linkage_new,
                                  mtype=linkage_new_types, btype=linkage_new_name)
            ...
            if self.verbose:
                print('Connect a {} to a {} unit (in polymer) via a {} bond'.format(
                    linkage_new_types[0], linkage_new_types[1], linkage_new_name))

            # set the new linkage flag
            new_linkage_flag = True

        # make other carbon bonding with oxygen if necessary unavailable
        if not alpha_index_in_polymer == None:
            self.G = ut.make_unavailable(self.G, alpha_index_in_polymer)
        if not alpha1_index_in_polymer == None:
            self.G = ut.make_unavailable(self.G, alpha1_index_in_polymer)
        if not alpha2_index_in_polymer == None:
            self.G = ut.make_unavailable(self.G, alpha2_index_in_polymer)
        if not C4_index_in_polymer == None:
            self.G = ut.make_unavailable(self.G, C4_index_in_polymer)
...
class Polymer(PolymerGraph):
    ...
    def update_units_5_5(
        ...
        self.bigG.add_edges_from(
            [(M1_index, M3_index)], btype=linkage_new_name)
        self.bigG.add_edges_from(
            [(M2_index, M3_index)], btype=linkage_new_name)
        # update 5-5 to 5-5-ring
        self.bigG.edges[(M1_index, M2_index)]['btype'] = linkage_new_name
    ...
    def update_units(
        ...
        # Add the edges to monomer nodes
        self.bigG.add_edges_from(
            [(M1_index, M2_index)], btype=linkage_new_name)
    ...
    def try_adding_new_5_5(
        ...
        # Add the linkage
        new_linkage_flag = PG_temp.connect_C1_C2_O_pair(
            O_pair_indices, C1_index_in_polymer, C2_index_in_polymer)
        ...
        if new_linkage_flag:
            ...
            # Update the avaiable O pairs for 5-5
            self.O_pair_indices_in_polymer = self.find_available_O_pairs_indicies_in_polymer()
            # Update the unit dictionary
            self.update_units_5_5(
                O_pair_indices, C1_index_in_polymer, C2_index_in_polymer)
            # Update the current monomer index
            self.mi += 1

        if draw:
            ut.draw_graph(self.G)
    ...
    def try_adding_new(
        ...
        # Here we create a temp PolymerGraph object to connect C1 and C2
        PG_temp = PolymerGraph(self.G.copy(), verbose=self.verbose)
        # PG_temp.O_pair_indices_in_polymer = self.O_pair_indices_in_polymer
        ...
        # Add the linkage
        new_linkage_flag = PG_temp.connect_C1_C2(
            linkage_index, C1_index_in_polymer, C2_index_in_polymer)
    ...
    def add_specific_linkage(
        ...
        # Add 5-5-ring if branching state is true
        self.O_pair_indices_in_polymer = self.find_available_O_pairs_indicies_in_polymer()

        if (linkage_type == '5-5') and (len(self.O_pair_indices_in_polymer)) > 0 and branching_state:
            ...
            C1_C2_pair_indices_in_monomer = ut.select_one_from_many(
                C1_C2_pairs_indices_in_monomer)

            # Find possible C2 index
            C1_index_in_polymer = self.find_C2_index_in_polymer(
                C1_C2_pair_indices_in_monomer[0])
            C2_index_in_polymer = self.find_C2_index_in_polymer(
                C1_C2_pair_indices_in_monomer[1])

            # Find possible O pair index
            O_pair_indices = ut.select_one_from_many(
                self.O_pair_indices_in_polymer)
        ...
        if len(C1_indices_in_polymer) == 0:
            ...
            C1_index_in_polymer = ut.select_one_from_many(
                C1_indices_in_polymer)
            C1_node = self.G.nodes[C1_index_in_polymer]
            ...
            monomer_types_possible = self.find_available_monomer_types(
                linkage_index)
    ...
    def add_specific_monomer(
        ...
        # Add 5-5-ring if branching state is true
        self.O_pair_indices_in_polymer = self.find_available_O_pairs_indicies_in_polymer()
        if (linkage_type == '5-5-ring') and (len(self.O_pair_indices_in_polymer) > 0) and branching_state:
            C1_C2_pairs_indices_in_monomer = [(7, 8), (8, 7)]
            C1_C2_pair_indices_in_monomer = ut.select_one_from_many(
                C1_C2_pairs_indices_in_monomer)
        ...
        if len(C1_indices_in_polymer) == 0:
            ...
            C1_indices_in_monomer_of_polymer = [
                self.G.nodes[ci]['index'] for ci in C1_indices_in_polymer]
            # find the C2 indices in the new monomer
            C2_indices_in_monmer_node = [
                n for n, v in Gm.nodes(data=True) if v['bonding']]
            C2_indices_in_monomer = [Gm.nodes[n]['index']
                                     for n in C2_indices_in_monmer_node]
            C2_indices_in_polymer = [self.find_C2_index_in_polymer(
                ci) for ci in C2_indices_in_monomer]
            ...
            linkage_possible_indices = [ind for ind, li in enumerate(
                linkage_indices_in_monomer_possible) if li in linkage_index_to_name.keys()]
            linkage_indices_in_monomer_possible = [
                linkage_indices_in_monomer_possible[ind] for ind in linkage_possible_indices]
            linkage_indices_in_polymer_possible = [
                linkage_indices_in_polymer_possible[ind] for ind in linkage_possible_indices]
            linkage_possible = [linkage_index_to_name[li]
                                for li in linkage_indices_in_monomer_possible]
            ...
            if len(linkage_indices_in_monomer_possible_input) == 0:
                ...
                linkage_index, mi = ut.select_one_from_many_with_index(
                    linkage_indices_in_monomer_possible_input)
    ...
    def add_random_monomer(
        ...
        C1_index_in_polymer = ut.select_one_from_many(
            self.C1_indices_in_polymer)

        C1_node = self.G.nodes[C1_index_in_polymer]
        C1_index_in_monomer = C1_node['index']

        # select the bond for the second C
        C2_indices_in_monomer = self.find_available_C2_in_monomer(
            C1_index_in_polymer)
    ...
    def add_random_ring(
        ...
        # Case 1, if there is no C1 available for bonding
        C1_indices_in_polymer = [
            ci for ci in self.C1_indices_in_polymer if self.G.nodes[ci]['index'] != 1]
        ...
        if len(C1_indices_in_polymer) == 0:
            ...
            C2_indices_in_polymer = self.update_available_C1_in_polymer(
                C1_index_in_polymer)
            C2_indices_in_monomer_of_polymer = [
                self.G.nodes[ci]['index'] for ci in C2_indices_in_polymer]

            # base on C1, select the possible C2 index in monomer and polymer
            C2_indices_in_monomer_possible = self.find_available_C2_in_monomer(
                C1_index_in_polymer, ring=True)
            ...
            if len(C2_index_in_monomer_of_polymer_possible) == 0:
                ...
                C2_index_in_monomer, pi = ut.select_one_from_many_with_index(
                    C2_index_in_monomer_of_polymer_possible)
                linkage_index = (C1_index_in_monomer, C2_index_in_monomer)
    ...
    def add_specific_ring(
        ...
        if len(C1_indices_in_polymer) == 0:
            ...
            # self.find_available_C2_in_monomer(C1_index_in_polymer, ring = True)
            C2_indices_in_monomer_possible = linkages_possible[C1_index_in_monomer]
            C2_index_in_monomer_of_polymer_possible = []
            C2_index_in_polymer_possible = []
