...
class Trajectory():
    ...
    def __init__(
        ...
        self.linkage_distribution = np.array(
            linkage_distribution_input)/np.sum(linkage_distribution_input)
        self.monomer_distribution = np.array(
        ...
        metrics_target = np.concatenate(
            (self.monomer_distribution, self.linkage_distribution), axis=0)
        ...
        if additional_metrics is not None:
            ...
            metrics_target = np.concatenate(
                (metrics_target, additional_metrics), axis=0)
    ...
    def run_MCMC(
        ...
        monomer_random = ut.generate_random_monomer(
        ...
        metrics_P, monomer_count_P, MW_P = ch.get_metrics_polymer(polymer,
                                                                  ...
                                                                  cal_MW=self.cal_MW)
        distance_init = ut.cal_distance(
            self.metrics_target, metrics_P, self.metrics_weights)

        '''
        Form a dimmer
        '''
        linkage_new = ut.generate_random_linkage(
            self.linkage_distribution, random_state)
        ...
        while (current_size <= self.stop_size) and (i_step <= i_max):
            ...
            if self.branching_propensity is not None:
                if self.branching_propensity > 0.0:  # randomly generate a branching state based on branching propensity
                    branching_state = ut.generate_random_branching_state(
    ...
    def run_MCMC_ring(
        ...
        while i_step <= i_max_ring:
            ...
            # Annealing version: (1 - i_step/i_max) * Tmax #
            Tmetro = self.Tmetro

            polymer_i = Polymer(polymer, verbose=False)  # self.verbose)
            new_linkage_flag = polymer_i.add_specific_ring(
                linkage_type=ut.generate_random_linkage(self.linkage_distribution, random_state))
...
class Simulation(Trajectory):
    ...
    def __init__(self,
        ...
        form_rings : Optional[bool], optional
            flag to control whether a ring would form inside a polymer to make it circular, by default True
        ...
        if not os.path.exists(ResultsPathParent):
            os.mkdir(ResultsPathParent)
    ...
    def run(self):
        ...
        if self.population_metrics is not None:
            population_metrics_normalized = [
                pi/self.max_MW for pi in self.population_metrics]
            metrics_target_to_match = np.append(
                metrics_target_to_match, population_metrics_normalized)
            metrics_target_to_match_original = np.append(
                metrics_target_to_match_original, self.population_metrics)

        self.metrics_target_dict = ut.metrics_array_to_dict(
            metrics_target_to_match_original, metrics_names_to_match)
        ...
        while n_polymers <= self.n_population and i_step <= self.i_max_out:
            ...
            P_i, distance_i, monomer_accepted_count, monomer_iteration_count = traj.run_MCMC(
                rseed, self.i_max)
            ...
            metrics_average_to_match = ut.counts_to_metrics(
                counts_sum, additional=self.additional)

            # add population metrics to match
            if self.population_metrics is not None:
                MW_average = ut.MW_array_to_number_average(
                    np.array(MW_population_copy))
                MW_weight_average = ut.MW_array_to_weight_average(
                    np.array(MW_population_copy))

                metrics_average_to_match = np.append(metrics_average_to_match, [
                                                     MW_average/self.max_MW, MW_weight_average/self.max_MW])

            # Compute the new distance
            d_average_new = ut.cal_distance(
                metrics_target_to_match, metrics_average_to_match, self.metrics_weights)
            ...
            if energy_flag:  # and distance_flag and NN_flag):
                ...
                file.write('\tn_polymer {} added on iteration no {} \n'.format(
                    n_polymers-1, i_step))
                if self.verbose:
                    print('\tn_polymer {} added on iteration no {}'.format(
                        n_polymers-1, i_step))
            else:
        ...
        file.write(
            'Runtime for creating all polymers : {:.2f} minutes \n'.format((end-start)/60))
        ...
        ring_count = 0
        ring_iterations = 0
        if self.form_rings and self.branching_propensity > 0.0:
            ...
            for Pi, ri in zip(P_population, rseeds):
                P_i, distance_i, acceptance_count, ring_its = traj.run_MCMC_ring(
                    Pi, ri, self.i_max_ring)
                ...
                metrics_w_ring_P = ut.counts_to_metrics(
                    counts_w_ring_P, additional=self.additional)
                file.write('\t{} ring(s) added to polymer {}\n'.format(
                    acceptance_count, polymer_no))
                if(acceptance_count > 0) and self.verbose:
                    print('\t{} ring(s) added to polymer {}'.format(
                        acceptance_count, polymer_no))
            ...
            file.write(
                'Runtime for adding the rings : {:.2f} minutes \n'.format((end-start)/60))
            print('Runtime for adding the rings : {:.2f} minutes \n'.format(
                (end-start)/60))

            # compute the metrics from the sum of counts
            counts_w_ring_sum = np.sum(
                np.array(counts_population_w_ring), axis=0)
            metrics_w_ring_to_match = ut.counts_to_metrics(
                counts_w_ring_sum, additional=self.additional)

            # add population metrics to match
            if self.population_metrics is not None:
                MW_w_ring_average = ut.MW_array_to_number_average(
                    np.array(MW_population_w_ring))
                MW_w_ring_weight_average = ut.MW_array_to_weight_average(
                    np.array(MW_population_w_ring))
                metrics_w_ring_to_match = np.append(metrics_w_ring_to_match, [
                                                    MW_w_ring_average/self.max_MW, MW_w_ring_weight_average/self.max_MW])

            d_w_ring_average = ut.cal_distance(
                metrics_target_to_match, metrics_w_ring_to_match, self.metrics_weights)
        ...
        if (self.trial_index == None):
            population = ch.Population(
                P_population, self.library_name, ResultsName=self.ResultsName)
        else:
            population = ch.Population(
                P_population, self.library_name, ResultsName=self.ResultsName, TrialIndex=str(self.trial_index))
        population.analyze()

        # the metrics average including branching coeff and MW
        self.metrics_current_dict = population.get_metrics_mean(
            additional=self.additional)
        ...
        if self.show_plots:
            ...
            metrics_population_to_plot = np.concatenate(
                (metrics_population, monomer_count_population, MW_population), axis=1)
            metrics_names_to_plot = monomer_types + linkage_names + \
                ['branching_coeff', 'monomer_count', 'MW']
            ...
            ut.plot_metrics(self.metrics_target_dict, self.metrics_current_dict,
                            metrics_population_to_plot, metrics_names_to_plot, self.ResultsPath)

            # Plot the population distance
            ut.plot_distance_trajectory(
                self.distance, self.library_name, 'Population', self.ResultsPath)

            # Plot the distance trajectory for the last polymer
            p_index = len(self.distance_polymer)-1
            ut.plot_distance_trajectory(
                self.distance_polymer[p_index], self.library_name + '_polymer_'+str(p_index), 'Pi', self.ResultsPath)
        ...
        print('\nAcceptance Rates')
        print('Monomer Acceptance: {}'.format(
            monomer_accepted/monomer_iterations))
        ...
        if (ring_iterations > 0):
            file.write('Ring Acceptance: {}\n'.format(
                ring_count/ring_iterations))
        file.write(
            'Runtime for analyzing the results : {:.2f} minutes \n'.format((end-start)/60))
        file.close()
...
class BOOptimizer():
    ...
    def optimize(
        ...
        # print(X_init)
        Y_init = bo.eval_objective_func(
            X_init, para_ranges_varying, objective_func_vectorized.predict)

        # Import the initial data
        Exp.input_data(
            X_init, Y_init, X_ranges=para_ranges_varying, unit_flag=True)
        Exp.set_optim_specs(
            objective_func=objective_func_vectorized.predict, maximize=False)
